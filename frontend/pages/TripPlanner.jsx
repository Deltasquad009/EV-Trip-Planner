...
export default function TripPlanner() {
  ...
  const estimatedCost = result ? (parseFloat(result.energy.totalEnergy) * costRate).toFixed(0) : null;
      ...
      <div style={s.layout}>
            ...
            <div style={s.resultsCard}>
              ...
              <div style={s.resultGrid}>
                {[
                  ...
                  { label: "Est. Cost", value: `₹${estimatedCost}`, icon: "💰", highlight: true },
              ...
              {/* Charging Timeline */}
              {result.energy.chargingStops > 0 && (
                <div style={s.timeline}>
                  <div style={s.timelineTitle}>⚡ Charging Journey</div>
                  <div style={s.timelineSteps}>
                    <div style={s.tlStep}>
                      <div style={{ ...s.tlDot, background: "#00e5a0" }} />
                      <div style={s.tlText}>
                        <span style={s.tlMain}>🚗 Start: {result.start}</span>
                        <span style={s.tlSub}>Battery: {form.batteryPercent}%</span>
                      </div>
                    </div>
                    {[...Array(result.energy.chargingStops)].map((_, i) => (
                      <div key={i} style={s.tlStep}>
                        <div style={s.tlLine} />
                        <div style={{ ...s.tlDot, background: "#f59e0b" }} />
                        <div style={s.tlText}>
                          <span style={s.tlMain}>🔌 Charging Stop {i + 1}</span>
                          <span style={s.tlSub}>Top up battery en-route</span>
                        </div>
                      </div>
                    ))}
                    <div style={s.tlStep}>
                      ...
                      <div style={s.tlText}>
                        <span style={s.tlMain}>🏁 Arrive: {result.destination}</span>
        ...
        <div style={s.mapWrapper}>
          <MapContainer
            ...
            zoomControl={false}
          >
            {/* Dark CartoDB tile — beautiful dark map */}
            <TileLayer
