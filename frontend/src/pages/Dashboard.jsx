...
export default function Dashboard() {
  ...
  const [user, setUser] = useState({ name: "EV Driver" });
  ...
  useEffect(() => {
    ...
    const fetchTrips = async () => {
        try {
            const tripsData = await getTripHistory();
            
            // Format dates and prepare trips array
            const formattedTrips = tripsData.map(trip => {
                const date = new Date(trip.createdAt);
                
                // Format distance safely handling potential string/number mismatches
                let distText = "Unknown";
                if (trip.distance != null) {
                    distText = `${Math.round(trip.distance)} km`;
                }

                return {
                    from: typeof trip.start === 'string' ? trip.start.split(',')[0] : "Unknown",
                    to: typeof trip.destination === 'string' ? trip.destination.split(',')[0] : "Unknown",
                    dist: distText,
                    battery: trip.batteryRemaining != null ? `${Math.round(trip.batteryRemaining)}%` : "Unknown",
                    date: date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
                    status: "Completed", 
                    rawDistance: trip.distance || 0,
                    stops: trip.chargingStops || 0
                };
            });
            ...
            const totalDistRaw = formattedTrips.reduce((sum, trip) => sum + (trip.rawDistance || 0), 0);
            const totalStops = formattedTrips.reduce((sum, trip) => sum + (trip.stops || 0), 0);
            
            setStats({
                tripsPlanned: formattedTrips.length,
                totalDistance: `${Math.round(totalDistRaw)} km`,
                // Assuming rough 0.15 kWh per km efficiency compared to standard ICE emissions for "saved" metric
                energySaved: `${Math.round(totalDistRaw * 0.15)} kWh`, 
                chargingStops: totalStops
            });
  ...
  const statCards = [
    { icon: "🗺️", label: "Trips Planned", value: stats.tripsPlanned.toString(), sub: "Total", color: "#00e5a0" },
    { icon: "🛣️", label: "Total Distance", value: stats.totalDistance, sub: "Lifetime", color: "#00bcd4" },
    { icon: "⚡", label: "Energy Impact", value: stats.energySaved, sub: "Estimated", color: "#a78bfa" },
    { icon: "🔌", label: "Charging Stops", value: stats.chargingStops.toString(), sub: "Total needed", color: "#f59e0b" },
  ];
